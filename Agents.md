...
- `Portic.Core` is the only packable runtime project. Its version comes from MinVer and `v`
  tags; never add a hand-maintained version. Run `pwsh scripts/Test-Package.ps1` to verify the
  produced package composes in a separate consumer with public dependencies (CI runs it too).
